...
<!-- ================ start banner area ================= -->	
<section class="contact-banner-area" id="contact" style='background: url("{{ asset('public/storage/images/resort/banner/'.$resort->bannerimage) }}") center center no-repeat;background-size: auto;'>
...
<script>
      ...
      $(".facility-more").click(function(e) {
            ...
            $.ajax({
                ...
                url: 'getprocess/experiencesmore.php',
                success: function(result) { //alert(result);

                    var obj = JSON.parse(result);
                    var len = result.length;
                    var name = obj.name;
                    var description = obj.discription;
		...
		$(".villa-more").click(function(e) {
            ...
            $.ajax({
                ...
                success: function(result) { //alert(result);
                    ...
                    var name = obj.name;
                    var description = obj.discription;
                    var roomsize = obj.roomsize;
                    var sleep = obj.sleep;
                    var view = obj.view;
                    var wifi = obj.wifi;
                    var ac = obj.ac;
                    var barthroom = obj.barthroom;
                    if (wifi == 1) {
                        var iswifi = '<i class="bi bi-wifi"></i>'
                    }
                    if (ac == 1) {
                        var isac = '<i class="bi bi-snow2"></i>'
                    }
                    if (barthroom == 1) {
                        var isbarthroom = '<i class="bi bi-textarea-resize"></i>'
                    }
                    $('.villa-title').html(name);
                    $('.villa-body').html('<div class="row g-0">' + description +
                        '</div><div class="row villa-icon mt-4"><div class="col-6 col-sm-6 col-md-6 col-lg-4"><p><i class="bi bi-house"></i> ' +
                        roomsize +
                        ' </p></div><div class="col-6 col-sm-6 col-md-6 col-lg-4"><p><i class="bi bi-people"></i> ' +
                        sleep +
                        '</p></div><div class="col-6 col-sm-6 col-md-6 col-lg-4"><p><i class="bi bi-eye"></i> ' +
                        view +
                        ' </p></div></div></div>');
                }
            });
